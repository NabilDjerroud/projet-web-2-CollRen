...
function UneVoiture(props) {
    ...
    console.log(voiture.carburant.type[language])
    ...
    return (
        ...
        <tr key={props?.id}>
            ...
            <td className="px-2 py-2 whitespace-nowrap">
                <Link to={`/voitures/${props.id}`} className="gap-x-[1rem] bg-[#F96C25] hover:bg-[#868E9B] text-white font-bold py-2 px-4 rounded-full">
                    {t("btnChoisir")}
                </Link>
